...
$fileLocation = Join-Path $toolsDir 'Backup4allLite86Setup.exe'
...
$url        = 'http://www.cyberdev.be/choco/Backup4allLite86Setup.exe' # download url, HTTPS preferred
$url64      = 'http://www.cyberdev.be/choco/Backup4allLite86Setup.exe' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url
...
$packageArgs = @{
  ...
  softwareName  = 'backup4all86*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
  ...
  #silentArgs   = '/S'           # NSIS
  silentArgs   = '/quiet /norestart' # Inno Setup
